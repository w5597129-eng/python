...
def on_publish(client, userdata, mid):
    # print(f"[MQTT] Message {mid} published.") # Uncomment for verbose publish feedback
    pass
...
def init_mqtt():
    ...
    try:
        ...
        mqtt_client.loop_start() # Start background thread for MQTT operations
    except Exception as e:
        print(f"[MQTT] Could not connect to MQTT broker: {e}")
    ...
    if cycle_count % PRINT_EVERY == 0:
        ...
        if mqtt_client:
            try:
                ...
                print(f"[MQTT] Published: {json.dumps(msg)}") # Keep print for local feedback
            except Exception as e:
