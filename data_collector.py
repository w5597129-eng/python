...
import RPi.GPIO as GPIO # For fan control
...
# TB6612FNG pins (BCM)
FAN_AIN1 = 27          # TB6612 AIN1
FAN_AIN2 = 22          # TB6612 AIN2
FAN_PWMA = 18          # TB6612 PWMA (PWM-capable)
FAN_STBY = 23          # TB6612 STBY (HIGH to enable)
...
def read_mpu(bus, addr):
    ACCEL_SENS, GYRO_SENS = 16384.0, 131.0 # Hardcoded values
...
def fan_init():
    ...
    GPIO.setup([FAN_AIN1, FAN_AIN2, FAN_PWMA, FAN_STBY], GPIO.OUT, initial=GPIO.LOW)
    pwm = GPIO.PWM(FAN_PWMA, PWM_FREQ)
    pwm.start(0)
    # standby release
    GPIO.output(FAN_STBY, GPIO.HIGH)
    fan_off()

def fan_on(duty=FAN_DUTY_ON):
    # Forward direction: AIN1=HIGH, AIN2=LOW
    GPIO.output(FAN_AIN1, GPIO.HIGH)
    GPIO.output(FAN_AIN2, GPIO.LOW)
    pwm.ChangeDutyCycle(max(0, min(100, duty)))

def fan_off():
    pwm.ChangeDutyCycle(0)
    GPIO.output(FAN_AIN1, GPIO.LOW)
    GPIO.output(FAN_AIN2, GPIO.LOW)
...
def main():
    ...
    while not stop_flag:
        ...
        # ---------- Vibration ----------
        if vib_ch and (now - last_vib >= INTERVAL_VIB):
            try:
        ...
        # ---------- Sound ----------
        if sound_ch and (now - last_sound >= INTERVAL_SOUND):
            try:
        ...
        if bmp and (now - last_press >= INTERVAL_PRESS):
            try:
                ...
                try:
                    ...
                    pass # Altitude might not be available or fail
                try:
                    slp_h = round(float(bmp.read_sealevel_pressure()) / 100.0, 2)
                except Exception:
                    pass # Sea level pressure might not be available or fail
